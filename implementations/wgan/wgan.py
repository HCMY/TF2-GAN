...
def wasserstein_loss(d_real_label, d_fake_label):
    d_loss = -tf.reduce_mean(d_real_label)+tf.reduce_mean(d_fake_label)
    g_loss = -tf.reduce_mean(d_fake_label)
...
d_optimizer = keras.optimizers.RMSprop(1e-4)
...
def train_one_step(images):
    ...
    with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:
        ...
        d_loss, g_loss = wasserstein_loss(d_real_label=real_pred, d_fake_label=fake_pred)
    ...
    for idx, grad in enumerate(d_grad):
        d_grad[idx] = tf.clip_by_value(grad, -0.01,0.01)
